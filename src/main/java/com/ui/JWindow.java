...
public class JWindow extends Serialize {
    ...
    public void update(double dt) {
        if (MouseListener.mouseButtonDown(GLFW_MOUSE_BUTTON_1)) {
            ...
            if (MouseListener.isDragging() && pointInRectangle(MouseListener.position(), this.position, titleBarSize) ||
                beingDragged) {
                if (!beingDragged) {
                    clickedPosDelta.x = MouseListener.getX() - this.position.x;
                    clickedPosDelta.y = MouseListener.getY() - this.position.y;
                    beingDragged = true;
                }
                this.position.x = MouseListener.getX() - clickedPosDelta.x;
                this.position.y = MouseListener.getY() - clickedPosDelta.y;
            } else if (MouseListener.isDragging() &&
                    pointInRectangle(MouseListener.position(), Vector2.minus(Vector2.add(this.position, this.size), this.draggableClickSize), draggableClickSize) ||
                    beingResized) {
                if (!beingResized) {
                    //Window.getWindow().setCursor(resizeCursor);
                    beingResized = true;
                }
                this.size.x = MouseListener.getX() - this.position.x;
                this.size.y = MouseListener.getY() - this.position.y;
                scrollbarPos.y = 0;
            }
        } else if (pointInWindow(MouseListener.position()) && MouseListener.getScrollY() != 0.0f && drawScrollbar) {
        ...
        for (Tab tab : tabs) {
            if (MouseListener.mouseButtonDown(GLFW_MOUSE_BUTTON_1) && tab.mouseInButton()) {
                for (Tab oTab : tabs) {
                    oTab.setInactive();
                }
                tab.setActive();
